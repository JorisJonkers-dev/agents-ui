#!/usr/bin/env bash
# render-local.sh — local CI-parity render for agents-ui.
#
# Mirrors what deploy-validate.yml / deploy-artifact.yml do in CI:
#   validate -> render -> kubeconform -> leak-scan -> scorecard
#
# Usage:
#   ./deploy/platform/render-local.sh [--diff] [--context-dir PATH] [--scorecard-only]
#
# Options:
...
# the caller overrides NPM_PROVENANCE_VERIFIED explicitly.
...
resolve_schema_version() {
  # Priority: SCHEMA_VERSION env > .platform/deploy-version file > baked-in template value.
...
npm_audit_and_scorecard_init() {
  # Failure is captured in the scorecard; it does not abort the render
  # (local dev ergonomics — CI enforces it hard in deploy-artifact.yml).
...
# ─── SC-11 scorecard ────────────────────────────────────────────────────────
# Pure shell + jq; runs without the npm package (dry mode / tests).

# Echo the deployment.yml content with comments stripped, so commented-out
# example blocks never influence the scorecard.
deployment_source() {
  sed 's/#.*$//' "$1"
}

scorecard_images_pinned() {
  # pass iff every ref in images.lock.json is digest-pinned (object or
  # transitional array form).
  local lock="$1" ref
  [ -f "$lock" ] || {
    echo "fail"
    return
...
scorecard_route_owner_authmode() {
  # not_applicable when no workload declares routes; otherwise pass iff both
  # owner and authMode are declared (route-level or routeDefaults).
  local src="$1"
...
scorecard_stateful_policy() {
  # not_applicable when all workloads are stateless; any stateful workload
  # without a migrationPolicy fails.
  local src="$1"
...
scorecard_raw_manifests() {
  # not_applicable unless a workload enables rawManifests; then pass iff the
  # raw-manifests guard file was produced by validate-raw-manifests.
  local src="$1"
...
write_scorecard_outputs() {
  {
    echo "# Deployment readiness scorecard — agents-ui"
    echo ""
    echo "| Check | Status |"
    echo "|-------|--------|"
...
# Only run when executed, so tests can source the scorecard functions.
