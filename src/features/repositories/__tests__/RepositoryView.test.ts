...
describe('repositoryView access status', () => {
  ...
  it('renders GitHub App owner guidance and canonical external links', async () => {
    getRepository.mockResolvedValue(detail({ repository: fakeRepo({ repoUrl: 'git@github.com:JorisJonkers-dev/demo.git' }) }))
    const wrapper = await mountView()

    expect(wrapper.find('[data-testid="github-app-owner"]').text()).toBe('JorisJonkers-dev')
    ...
    const expectedLinks = {
      'github-app-install-link': 'https://github.com/apps/jorisjonkers-dev-agents/installations/new?state=JorisJonkers-dev',
      'github-app-user-installations-link': 'https://github.com/settings/installations',
      'github-app-organization-installations-link':
        'https://github.com/organizations/JorisJonkers-dev/settings/installations',
