...
describe('githubAppLinks', () => {
  ...
  it.each([
    { owner: 'JorisJonkers-dev', expected: true },
  ...
  it('builds the canonical GitHub App installation URL with owner state', () => {
    expect(buildGitHubAppInstallationUrl('JorisJonkers-dev')).toBe(
      'https://github.com/apps/jorisjonkers-dev-agents/installations/new?state=JorisJonkers-dev',
    )
    expect(buildGitHubAppInstallationUrlForRepo('git@github.com:JorisJonkers-dev/agents.git')).toBe(
      'https://github.com/apps/jorisjonkers-dev-agents/installations/new?state=JorisJonkers-dev',
    )
  })

  it('safely encodes the app slug path segment', () => {
    expect(buildGitHubAppInstallationUrl('JorisJonkers-dev', 'custom-app')).toBe(
      'https://github.com/apps/custom-app/installations/new?state=JorisJonkers-dev',
    )
    expect(() => buildGitHubAppInstallationUrl('JorisJonkers-dev', 'custom/app')).toThrow(
